...
const static char *FLAG_ID = "Cuc";
uint32_t SERIAL_NUMBER = 1234567;
...
//static gxObject* NONE_OBJECTS[] = { BASE(associationNone), BASE(ldn), BASE(eeprom), BASE(sapAssignment), BASE(hodiny) };
...
uint32_t time_current(void)
{
  ...
  return time_toUnixTime2(&hodiny.time);
#else
  uint32_t tmp = time_elapsed() - started;
  tmp /= 1000;
  tmp += time_toUnixTime2(&hodiny.time);
  return tmp;
#endif //USE_TIME_INTERRUPT
}
...
void time_now(gxtime* value, unsigned char meterTime)
{
  ...
  if (meterTime)
  {
    clock_utcToMeterTime(&hodiny, value);
  }
#endif //USE_TIME_INTERRUPT
}
...
int addClockObject()
{
  ...
  if ((ret = INIT_OBJECT(hodiny, DLMS_OBJECT_TYPE_CLOCK, ln)) == 0)
  {
    //Set default values.
    time_init(&hodiny.begin, -1, 3, -1, 2, 0, 0, 0, 0);
    hodiny.begin.extraInfo = DLMS_DATE_TIME_EXTRA_INFO_LAST_DAY;
    time_init(&hodiny.end, -1, 10, -1, 3, 0, 0, 0, 0);
    hodiny.end.extraInfo = DLMS_DATE_TIME_EXTRA_INFO_LAST_DAY;
    //Meter is using UTC time zone.
    hodiny.timeZone = 0;
    //Deviation is 60 minutes.
    hodiny.deviation = 60;
    hodiny.clockBase = DLMS_CLOCK_BASE_FREQUENCY_50;
  }
  return ret;
}
...
int addLoadProfileProfileGeneric()
{
  ...
  if ((ret = INIT_OBJECT(profileGeneric, DLMS_OBJECT_TYPE_PROFILE_GENERIC, ln)) == 0)
  {
    ...
    //Add clock object.
    CAPTURE_OBJECT[0].target = BASE(hodiny);
    ...
    profileGeneric.sortObject = BASE(hodiny);
...
int addEventLogProfileGeneric()
{
  ...
  if ((ret = INIT_OBJECT(eventLog, DLMS_OBJECT_TYPE_PROFILE_GENERIC, ln)) == 0)
  {
    ...
    CAPTURE_OBJECT[1].dataIndex = 0;
    //Set clock to sort object.
    eventLog.sortObject = BASE(hodiny);
  ...
  gxValueEventCollection* args)
{
  ...
  for (pos = 0; pos != args->size; ++pos)
  {
    ...
    if (e->target == &frekvence.base && e->index == 2)
    {
      readfrekvenceValue();
    }
    //Get time if user want to read date and time.
    if (e->target == BASE(hodiny) && e->index == 2)
    {
#ifndef USE_TIME_INTERRUPT
      gxtime dt;
      time_now(&dt, 1);
  ...
  gxValueEventArg* it)
{
  ...
  else if (it->index == 2)
  {
    ...
    row.frekvence = readfrekvenceValue();
  ...
  gxValueEventCollection * args)
{
  ...
  for (pos = 0; pos != args->size; ++pos)
  {
    ...
    else if (e->target == BASE(frekvence))
    {
      //Set default value for active power.
      frekvenceValue = 0;
      e->handled = 1;
